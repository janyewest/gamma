...
from Supabase.config.secrets_util import get_key_from_keychain

def get_data(url: str, query_ct: str, query_events: str, orderby: list[str], max_results: int):
...
def flatten_data(count, list_gw):
    ...
    # format instruments
    def normalize_instruments(insts):
        all_dets = ['H1', 'L1', 'V1']
        return [det if insts and det in insts else '--' for det in all_dets]
...
def to_supabase_db(outfile: str, url: str, table: str, flat_data):
    """JSON to Supabase database table"""
    
    print('\nConnecting to Supabase...')
    key = get_key_from_keychain('supabase-api-key', 'jayneharper')
...
if __name__ == '__main__':
    ...
    supabase_url = 'https://qltbxmdkcuoxdpoqtekl.supabase.co'
    ...
    # get data
    count_gw, events_gw = get_data(gracedb_url, query_count, query_gw, orderby_gw, max_results_gw)
    ...
    # to Supabase table
    to_supabase_db(output_file_json, supabase_url, supabase_table, flat_events_gw)
